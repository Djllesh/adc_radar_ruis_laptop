...
def fetch_data( ):
    ...
    inputsTrain = np.zeros((100, channelA.shape[0]))
    labelsTrain = np.zeros((100, 256*256))

    inputsPred = np.zeros((10, channelA.shape[0]))
    labelsPred = np.zeros((10, 256*256))
    ...
    while N < 100:
        ...
        if has_radar:
            if old_viscam_cnt != shm_viscam_meta[0]:
                ...
                viscam_frame = shm_viscam_data[:, :].copy()
                labelsTrain[N, :] = viscam_frame.flatten().astype(np.float32) / 255
    ...
    while N < 10:
        ...
        if has_radar:
            if old_viscam_cnt != shm_viscam_meta[0]:
                old_viscam_cnt = shm_viscam_meta[0]
                viscam_frame = shm_viscam_data[:, :].copy()
                labelsPred[N, :] = viscam_frame.flatten().astype(np.float32) / 255
