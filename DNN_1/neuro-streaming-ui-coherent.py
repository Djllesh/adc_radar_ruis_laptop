...
shm_radar_meta = np.ndarray((16,), dtype=np.uint32, buffer=shm_radar.buf )
shm_radar_data = np.ndarray((1, samplesPerBuffer), dtype=np.uint16, buffer=shm_radar.buf, offset = shm_radar_meta.nbytes)
...
channelA = (shm_radar_data[0, :] >> bitshift).astype(np.float32)
...
def fetch_data( N_train = 100, N_validate = 10 ):
    ...
    channelA = channelA[startSample:stopSample].copy()
    channelA = scipy.signal.sosfilt( filtteri, channelA) * 1000
    ...
    inputsTrain = np.zeros((N_train, channelA.shape[0]))
    #labelsTrain = np.zeros((N_train, 256*256))
    labelsTrain = np.zeros((N_train, 64*64))

    inputsPred = np.zeros((N_validate, channelA.shape[0]))
...
class App( object ):
    def __init__(self, root):
        ...
        self.btn2 = tki.Button( self.control_frame, text= "Save data", command=self.dump_to_disk )
    ...
    def dump_to_disk( self ):
        filename = f"targetSynthesis_cube_58cm_dense_{self.counter}.hdf5"
        fn = tki.filedialog.asksaveasfilename( filetypes = [("HDF5 file", "*.hdf5"), ("All files", "*.*")], defaultextension = ".hdf5" , initialfile = filename)
        if len( fn ) > 0:
            Npoints = pymsgbox.prompt(text = "Number of frames", title = "Action required", default="1000")
            Npoints = int(Npoints)
