...
class App( object ):
    def __init__(self, root):
        ...
        self.mittari = self.rm.open_resource( 'GPIB0::13::INSTR' )

        self.root.option_add( "*Font", "B612" )
        self.root.title("MilliScan Power Meter Controller")
        ...
        self.actionmenu.add_command(label = "Reset power meter", command=self.handle_reset_power_meter )
    ...
    def handle_reset_power_meter( self ):
        self.mittari.query("*RST")

    def select_file( self ):
        fn = tki.filedialog.asksaveasfilename( filetypes = [("CSV file", "*.csv"), ("All files", "*.*")], defaultextension = ".csv" )
        if fn is not None:
            if len(fn) > 1:
    ...
    def run_sweep( self ):
        ...
        for i in range( n_points ):
            ...
            power = float( self.mittari.query("") )

            print( f, power )


            if len(fn) > 0:
                print( "%f %f" % (f, power), file = fn_handle)
